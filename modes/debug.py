...
def window_center():
    ...
    lines.append('Green: '.ljust(justnum) + utils.add_text_tag('NORMAL ', config.TAGS['green']) + utils.add_text_tag(' BRIGHT ', config.TAGS['bright_green']) + utils.add_text_tag(' NORMAL ', fg = config.TAGS['background'], bg = config.TAGS['green']) + ' ' + utils.add_text_tag(' BRIGHT ', fg = config.TAGS['background'], bg = config.TAGS['bright_green']))
    ...
    lines.append('')
    lines.append('INPUT:')
    lines.append('Last input device: '.ljust(justnum) + str(config.last_input_device).capitalize())
    lines.append('Last input: '.ljust(justnum) + str(config.last_input))
    lines.append('')
    lines.append('AUDIO:')
    lines.append('Music playing: '.ljust(justnum) + str(audio.music_status()))
    lines.append('Music volume: '.ljust(justnum) + str(config.settings['music_volume']))
    lines.append('Music mode: '.ljust(justnum) + str(audio.music_type).capitalize())
    lines.append('Music title: '.ljust(justnum) + str(audio.music_title))
    lines.append('')
    lines.append('GAME:')
    lines.append('Luck skill level: '.ljust(justnum) + str(config.player['luck']))
    lines.append('')
    ...
    lines.append('Number of drinks player has consumed: '.ljust(justnum) + str(config.stats['drinks_consumed']))
    lines.append('Number of foods player has consumed: '.ljust(justnum) + str(config.stats['foods_consumed']))
    lines.append('Number of books player has consumed: '.ljust(justnum) + str(config.stats['books_consumed']))
    lines.append('Successfull hits thanks to luck: '.ljust(justnum) + str(config.stats['times_player_attack_luck']))
    lines.append('Successfull dodges thanks to luck: '.ljust(justnum) + str(config.stats['times_npc_missed_luck']))
    lines.append('')
    lines.append('FLAGS:')
