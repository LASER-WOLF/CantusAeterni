...
def check_rooms():
    ...
    global max_x
    global max_y
    known_rooms = portal_check({'selected_room': system.active_room[0], 'current_coords': {'x': 0, 'y': 0}, 'checked_rooms': {}})['checked_rooms']
...
def portal_check(portal_check_dict):
    ...
    for portal_id in system.rooms[selected_room_root]['portal']:
        portal = system.portals[portal_id]
        if portal.disabled == False:
            next_room = portal.room_2nd
            next_dir = portal.dir_1st_to_2nd
            next_portal_pos = portal.pos_2nd
            if next_room == selected_room_root:
                next_room = portal.room_1st
                next_dir = utils.DIRECTION_REVERSE[next_dir]
                next_portal_pos = portal.pos_1st
    ...
    for y in range(max_y):
        ...
        for x in range(max_x):
            ...
            if {'x': x, 'y': y} in known_rooms.values():
                ...
                selected_room = config.ui_selection_current[0]
